...
RUN DEBIAN_FRONTEND=noninteractive apt-get update -y && DEBIAN_FRONTEND=noninteractive apt-get -yq upgrade && \
    python -m pip install --upgrade pip && \
    python -m pip install pikepdf


FROM --platform=linux/arm64 python:3.8-slim-buster as stage-arm64
ARG TARGETOS
ARG TARGETARCH
ARG TARGETVARIANT
RUN echo "I'm building for $TARGETOS/$TARGETARCH/$TARGETVARIANT"
...
FROM --platform=linux/arm/v6 python:3.8-slim-buster as stage-armv6
ARG TARGETOS
ARG TARGETARCH
ARG TARGETVARIANT
RUN echo "I'm building for $TARGETOS/$TARGETARCH/$TARGETVARIANT"
...
## qpdf and pikepdf need to be built from source on armv6
RUN set -x && \
    TEMP_PACKAGES=() && \
    KEPT_PACKAGES=() && \
    # Packages only required during build
...
FROM stage-${TARGETARCH}${TARGETVARIANT}
